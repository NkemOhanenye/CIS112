...
public class ArrayListMap<K, V>  implements MapInterface<K,V>
{
  ...
  // If an entry in this map with key Move already exists then the value
  // associated with that entry is replaced by value v and the original
  // value is returned; otherwise, adds the (Move, v) pair to the map and
  // returns null.
  {
    if (k == null)
    ...
    while (search.hasNext())
    {
      ...
      if (temp.getKey().equals(k))
      {
        search.remove();
        map.add(entry);
        return temp.getValue(); // Move found, exits method
      }
    }
    
    // No entry is associated with Move.
    map.add(entry);
    return null;
  }

  public V get(K k)
  // If an entry in this map with a key Move exists then the value associated
  ...
  public V remove(K k)
  // If an entry in this map with key Move exists then the entry is removed
  ...
  // Returns true if an entry in this map with key Move exists;
  // Returns false otherwise.
  {
    ...
    for (MapEntry<K,V> temp: map)
      if (temp.getKey().equals(k))
        return true;     // Move found, exits method
