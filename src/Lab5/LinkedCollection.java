 ...
 * Date: 02/20/18
 * Purpose: To add a tostring, count, removeAll. and combine method
 */
...
public class LinkedCollection<T> {
    ...
    public String toString(){           //creates and returns a string of the contents of the collection
    ...
    public int count(T target){         //does the same as the size() function
    ...
    public void removeAll(T target){                //removes all nodes and contents

        find(target);
        if (found) {
            find(target);
            if (head == target)
                head = head.getLink();    // removes all nodes
